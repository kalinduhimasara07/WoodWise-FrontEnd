...
import { Edit, Upload, X, Save, Eye, Box } from "lucide-react";
import BackButton from "../../components/backButton";
import toast from "react-hot-toast";

export default function EditTimber() {
  const location = useLocation(); // to get the state from the navigation
  const navigate = useNavigate();

  const timber = location.state; // Get the timber data passed from the previous page

  const [formData, setFormData] = useState({
    category: timber ? timber.category : "",
    grade: timber ? timber.grade : "",
    pricePerUnit: timber ? timber.pricePerUnit : "",
    description: timber ? timber.description : "",
    dimensions: {
      length: timber ? timber.dimensions.length : "",
      width: timber ? timber.dimensions.width : "",
      height: timber ? timber.dimensions.height : "",
    },
    stock: timber ? timber.stock : "",
    sku: timber ? timber.sku : "",
    inStock: timber ? timber.inStock : true,
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success("Timber item updated successfully!");
        // Navigate back to the inventory page or wherever appropriate
        navigate("/mill/inventory");
      } else {
        toast.error("Error: " + result.message);
      }
    } catch (error) {
      toast.error("Error updating timber item");
    } finally {
