...
const MessageComponent = ({
  ...
  const getSenderInfo = (senderId) => {
    const senderMap = {
      ...
      storeStaff: { name: 'Store Staff', color: 'bg-blue-100 text-blue-800', icon: '🏪' },
      millStaff: { name: 'Mill Staff', color: 'bg-green-100 text-green-800', icon: '🏭' }
    };
    return senderMap[senderId] || { name: senderId, color: 'bg-gray-100 text-gray-800', icon: '👤' };
  };
      ...
      <div className={`max-w-xs lg:max-w-md ${isOwnMessage ? 'order-1' : 'order-2'}`}>
        ...
        {!isOwnMessage && (
          <div className="flex items-center mb-1">
            ...
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${senderInfo.color}`}>
              {senderInfo.name}
            </span>
          </div>
        )}
        ...
        {!isRead && !isOwnMessage && (
          <button
            ...
            className="mt-2 text-xs text-green-600 hover:text-green-800 transition-colors duration-200 flex items-center space-x-1 disabled:opacity-50 cursor-pointer"
          >
            {isMarkingAsRead ? (
          ...
          <div className="mt-2 text-xs text-right">
            <details className="cursor-pointer">
              <summary className="text-gray-500 hover:text-gray-700 transition-colors">
                Read by {message.readers.length} user{message.readers.length !== 1 ? 's' : ''}
              </summary>
              <div className="mt-1 space-y-1">
                {message.readers.map((readerId, index) => (
                  <div key={index} className="flex items-center justify-end space-x-1">
                    <span className="text-gray-600">{getSenderInfo(readerId).name}</span>
...
const MessageInput = ({ onSendMessage, currentUserId, disabled = false, isSending = false }) => {
...
const MillMessages = () => {
  ...
  if (loading) {
    return (
      <div className="mx-auto p-6 bg-gray-50 w-full h-full rounded-4xl overflow-auto">
        <div className="bg-white rounded-lg shadow-sm flex items-center justify-center" style={{ height: '580px' }}>
  ...
  return (
    <div className="mx-auto px-15 py-6 bg-gray-50 w-full h-full rounded-4xl overflow-auto">
      <div className="bg-white rounded-lg shadow-sm mb-6 flex flex-col" style={{ height: '580px' }}>
        {/* Header */}
