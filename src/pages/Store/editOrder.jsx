...
export default function EditOrder() {
  ...
  // Initialize furniture items from the passed order data
  const [furnitureItems, setFurnitureItems] = useState(
    orderData?.furnitureItems || [
      { sku: "", quantity: 1, unitPrice: "", note: "" },
    ]
  );

  useEffect(() => {
    // Redirect if no order data is passed
    if (!orderData) {
      toast.error("No order data found. Redirecting...");
      navigate("/orders"); // Adjust the path according to your routing
      return;
    }
  ...
  // Initialize order state with existing data
  const [order, setOrder] = useState({
    ...
    orderedBy: orderData?.orderedBy || "Store Manager",
    status: orderData?.status || "Pending",
    millWorker: orderData?.millWorker || "Not Assigned",
  ...
  const handleSubmit = async (e) => {
    ...
    // Construct the updated order payload
    ...
    try {
      ...
      toast.success(`Order ${orderData.orderNumber} updated successfully!`, {
        style: {
      ...
      // Navigate back to orders list or order details page
      navigate("/store/orders"); // Adjust the path according to your routing
    } catch (error) {
    ...
    <div className="w-full mx-auto p-8 bg-white shadow-md rounded-3xl overflow-auto">
      ...
      <div className="mt-3 px-20">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">
          Edit Order - {orderData?.orderNumber}
        </h2>
