    ...
    const fetchFurniture = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/furniture?${params.toString()}`
        );
        if (!response.ok) {
          ...
          <nav className="flex space-x-2 md:space-x-6 justify-center py-3 px-2">
            {[
              ...
              <button
                ...
                className={`relative flex items-center gap-2 py-2 px-5 rounded-xl font-semibold text-base capitalize transition-all duration-200
          ${
            activeTab === tab.id
